...
from panels import ChatPanel, NotesPanel
from utils import (
    AVAILABLE_MODELS,
    CACHE_EXPIRY_HOURS,
    MODEL,
...
class PDFReaderHelper(QMainWindow):
                ...
                if cached_entry:
                    ...
                    if datetime.now() - cached_time < timedelta(hours=CACHE_EXPIRY_HOURS):
                        ...
                        try:
                            ...
                            self.uploaded_file = self.gemini_client.files.get(name=cached_entry["file_name"])
                ...
                if not self.uploaded_file:
                    ...
                    try:
                        ...
                        self.uploaded_file = self.gemini_client.files.upload(
                            file=pdf_io, config={"mime_type": "application/pdf"}
                        )
    ...
    def set_model_as_default(self):
        ...
        QMessageBox.information(self, "Default Set", f"✓ {self.selected_model} is now your default model.")

    def set_zoom_as_default(self):
        """Save current zoom as user's default."""
        self.config["default_zoom"] = self.zoom
        save_config(self.config)
        QMessageBox.information(self, "Default Set", f"✓ Zoom level {int(self.zoom * 100)}% is now your default.")
...
- You have access to the full document - use it to provide context-aware answers"""
    ...
    def send_to_gemini(
        self, prompt: str, display_text: str = None, include_pdf: bool = False, image_bytes: bytes = None
    ):
        """Send a message to Gemini with streaming response."""
        if not self.gemini_client:
            QMessageBox.warning(
                self, "API Key Missing", "Please set GEMINI_API_KEY environment variable or add it to .env file."
            )
            return
